...
- **복구 문구를 두 기기 모두 아무도 받아 적지 않았다.** 합성 계정이고 항목이
  ...
  실제 대화 연결 전에 정해야 한다. 후보는 세 가지다. (A) 확인을 2단계로 만들어
  몇 개 단어를 되묻는다. (B) 연결 뒤에도 일정 기간 다시 볼 수 있게 한다 —
  다만 그동안 문구가 화면에 다시 나올 수 있다는 뜻이다. (C) 현행 유지하고
  경고 문구만 강화한다. 이것은 제품 결정이므로 Codex와 사용자가 정한다.
