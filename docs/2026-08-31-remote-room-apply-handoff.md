...
대화는 공간이 소유한다(`conversation_scope = (space_id, room_id, worldline_id?)`).
따라서 원격 방의 로컬 파일은 애초에 존재하지 않으며 **기존 대화에 병합할 일이
없다.** 병합 알고리즘이 없으면 병합 버그도 없다.

자기 공간 행은 읽고 대조만 하고 **절대 적용하지 않는다.**

### 8.1.1 이어가기 — 초안이 틀렸고 2026-08-31에 바로잡았다

설계 초안은 원격 방을 "완전한 읽기 전용"으로 적었다. **이것은 확정된 사용자 결정과
충돌하는 잘못이었다.** 결정 6번은 기존 방 이어가기를 허용하고, 결정 8번 각주는
읽기 전용이 검토자 권고였고 **사용자가 그것과 다른 선택을 했다**고 명시한다.
결정 15번은 수정·삭제를 막는 것이지 답장을 막는 것이 아니다.

기술 제약(delegated writer 없음)은 실재하지만, 그것은 "할 수 없다"가 아니라
"어떻게 하는가"의 문제였다. 확정된 방식은 이렇다.

> **답장은 답하는 기기가 자기 공간에 같은 `room_id`로 쓴다.**
> 한 방의 대화는 여러 공간의 합집합이며 화면에서 시각 기준으로 정렬해 합친다.

**이 작업자에게 주는 교훈.** 제품 결정과 기술 제약이 부딪히면 결정을 바꾸지 말고
**사용자에게 선택지를 제시한다.** 결정은 사용자 것이다.

### 8.2 저장 위치와 저장 identity

원격 방은 `sync/remote/` 아래 **별도 저장소**에 둔다. 기존 `ChatStore`의 파일,
검색 색인, 저장 스케줄러를 건드리지 않는다. 원격 적용 코드에 사용자 대화 파일로
가는 **경로 자체를 두지 않는다.**

저장·조회·원자적 교체·hash 계산의 단위는 **`room_id`가 아니라**:

| 대상 | 단위 |
| --- | --- |
| 방 메타 | `(space_id, room_id)` |
| turn·말풍선 | `(space_id, room_id, worldline_key)` (null은 `''`) |

§8.1.1의 이어가기를 열면 같은 `room_id`가 여러 공간에 있는 것이 정상이 된다.
화면에 보이는 "한 방"은 여러 저장 단위의 합집합이고, 합치는 일은 **화면 계층에서만**
한다. 저장 계층은 공간을 절대 합치지 않는다.

화면 정렬: 말풍선 `timestamp` 오름차순 → 같으면 `space_id` 사전순 → 그래도 같으면
`bubble_order`. `bubble_order`는 공간별이라 전체 순서를 주지 못한다.
...
**원격 저장소 자체는 읽기 전용으로 둔다.** 이것은 사용자 권한이 아니라 저장
계층의 성질이다. 답장은 원격 저장소가 아니라 **자기 공간 outbox로** 나가고
서버를 거쳐 다시 들어온다. 원격 저장소에 쓰는 코드는 조립기 하나뿐이다.
...
5. **tombstone 제외** — 아래 8.5.1
6. 조립 — `(space_id, room_id, worldline_key)` 단위, `bubble_order` 정렬,
   **원자적 교체**(임시 파일 + rename)
7. 대조 — 아래 8.5.2
...
`turn`·`bubble` projection에 `is_tombstoned`가 있다. 무시하면 **삭제된 말풍선이
원격 방에서 되살아난다.** 참인 행은 조립에서 빼고, **개수와 hash도 뺀 뒤** 계산
한다(양쪽이 같은 규칙이라야 대조가 의미를 갖는다). 필드 자체가 없는 행은 적용을
거부한다 — 없는 것을 거짓으로 간주하면 삭제가 조용히 되살아난다.

**지금은 tombstone이 존재할 수 없다.** `delete_turn`이
`RUNTIME_ENABLED_OPERATIONS`에서 빠져 있다(결정 15). 그래서 기존 검증 수치는
영향받지 않았다. 규칙과 시험은 지금 넣는다.

#### 8.5.2 대조는 아직 수동이다

개수·hash는 writer의 **로컬 manifest에만** 있고 wire에 실리지 않는다. 실기기
확인 때 대조한 것도 사람이 두 화면을 눈으로 비교한 것이었다.

| 단계 | 방식 |
| --- | --- |
| G1~G3 | 시험이 fixture expected와 비교 (자동) |
| **G4** | **사람이 두 화면을 비교 (수동)** |
| 자동 대조 | **미구현.** wire manifest 계약 필요 |

받는 기기는 자기 hash를 **표시**하되 일치 여부를 스스로 판정하지 않는다.
**판정하지 않은 것을 판정한 것처럼 표시하지 않는다.**
...
- **이어가기 구현** — 방식은 정해졌으나 미정 항목이 남아 G6에서 별도 승인
- **delegated writer** — 다른 공간의 행을 직접 고치기. 계약 없음, 만들지 않는다
- **개수·hash 자동 대조** — wire manifest 계약 필요 (§8.5.2)
