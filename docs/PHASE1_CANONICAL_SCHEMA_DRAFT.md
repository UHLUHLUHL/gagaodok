...
M05 D1은 6개 state enum 자체를 저장하지만 상태 전이 순서는 handler가 강제한다. `POST /v1/sync/operations`의 `create_attachment`가 유일한 allocation 경로이며, `allocated` 행과 operation/change ledger를 같은 atomic batch에서 만든다. 이때 생성한 난수 `r2_object_key`는 후속 content handler가 D1에서 조회하는 내부 값이고 client에 반환하지 않는다. 별도 attachment allocation endpoint는 두지 않는다. origin space device의 upload는 D1 `ciphertext_hash`를 R2 SHA-256 checksum option으로 전달하는 create-only conditional PUT으로 bytes를 검증하고 기존 object를 덮어쓰지 않으며, 성공 뒤 `uploaded`, R2 `head()`의 byte size 확인 뒤 `ready`가 된다. Worker 자체는 body를 buffer하거나 hash하지 않는다. `uploaded`는 cursor를 소비하지 않고, 다른 device가 관찰해야 하는 `ready` 전이만 새 `server_seq`와 revision-null attachment change event를 만든다. Download는 같은 account의 모든 유효 device에 허용한다. `abandoned`·`tombstoned`·`garbage_collected`도 같은 행에 남겨 audit와 dangling reference 검사를 유지하며 physical row delete를 lifecycle 의미로 사용하지 않는다.
