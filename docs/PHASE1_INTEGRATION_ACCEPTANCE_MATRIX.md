...
**표시등은 두 플랫폼이 같다 (2026-09-02).** 상태 앞의 점이 **켜져 있으면 초록,
아니면 회색**이다. 판정은 `SyncRuntimeStatus.isActive`로 양쪽이 공유하며
`idle`·`running`만 활성으로 본다.

`running`만 초록으로 두지 않은 이유가 있다. 실제로 도는 순간은 아주 짧아서,
그렇게 하면 켜져 있는 대부분의 시간에 꺼진 것과 구별되지 않는다.

회색 점은 두 플랫폼 모두 화면에서 확인했다(macOS 9/1, Android 9/2). **초록은
동기화를 켜야 나오므로 아직 화면에서 보지 못했다** — 합성 원격에 연결하는
단계에서 확인한다.

### ✅ 화면에서 확인함 — Gemini 3.8과 대화 요약 (2026-09-02~03)

**대화 요약 화면** (Android, ⋮ → 대화 요약). 세 방에서 확인했다.

| 방 | 표시등 | 상태 |
| --- | --- | --- |
| 마린(맥) | 초록 | 3구간 · 1~150턴 / 전체 191턴 · 원문 41턴 |
| 유키 | 초록 | 2구간 · 1~100턴 / 전체 141턴 · 원문 41턴 |
| 마린 | 회색 | 요약 없음 · 56턴 (80턴 임계값 전) |

구간을 눌러 요약 본문이 펼쳐지는 것, 회색·초록이 갈리는 것, 빈 상태 안내가
임계값과 현재 턴 수를 함께 보여주는 것을 모두 화면에서 봤다.

**Gemini 3.8 Flash**. 모델 메뉴에 3.8과 3.7이 함께 나오고, 3.8을 고르면 체크가
실제로 옮겨진다. 이것이 그냥 표시 확인이 아닌 이유는 아래에 있다.

**모델 선택이 원래 저장되지 않았다.** `AppSettings`가 선택을 preferences에 쓰기만
하고 읽지 않았고(초기값이 3.7로 박혀 있었다), `setSelectedModel`은 인자를 버리고
항상 3.7을 넣었다. 모델이 하나뿐이라 드러나지 않았을 뿐이다. 둘 다 고쳤고 화면에서
체크가 옮겨지는 것으로 확인했다.

기존 기기는 그 망가진 setter가 써둔 `gemini-3.7-flash`를 읽으므로 처음에는 3.7로
보인다. 한 번 3.8을 고르면 그때부터 유지된다. 새 설치는 3.8이 기본이다.
