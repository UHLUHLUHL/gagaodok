...
M03~M05, device token 인증, M06 ledger DDL과 patch_room first slice는 승인됐다. 다음은 `create_room`부터 entity family별 transaction service를 확장하고, 공용 dispatch가 계약과 일치한 뒤 §2.2 response envelope를 연결한다.
