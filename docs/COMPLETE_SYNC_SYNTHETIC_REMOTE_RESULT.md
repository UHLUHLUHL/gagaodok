...
#### 2026-09-03에 닫았다 — 객체 하나로 두 방향을 다 봤다

**첫 시도는 무효였다.** `wrangler r2 object put`을 `--remote` 없이 불러 고아가
로컬 모의 저장소에만 올라갔다. 그래서 `get --remote`가 "키가 없다"고 답한 것은
유지보수가 지운 결과가 아니라 **애초에 거기 없었던 결과**였다. 로컬에 53바이트가
그대로 남아 있는 것으로 확인했다. **R2 명령에는 `--remote`를 반드시 붙인다.**

다시 했다. 이번에는 24시간을 새로 기다리지 않고, **이미 28시간 된 실제 객체**
`obj/46BA6731-5613-443E-ABA4-1548CBC7FFF0`을 썼다. 나이는 그대로 두고 참조 여부만
바꿨다.

| 시점 | 그 객체의 상태 | cron 판정 |
| --- | --- | --- |
| 9/2 01:45 ~ 9/3 15:04 (28시간) | 오래됨 · **참조됨** | **보존** (cron 여러 번 통과) |
| 9/3 15:04 D1의 `r2_object_key` 변경 | 오래됨 · **고아** | — |
| 9/3 15:17 cron | | **삭제** (`key does not exist`) |

같은 객체이고 나이도 같다. **바뀐 것은 참조 여부뿐이다.** 그래서 판정 근거가
나이 필터가 아니라 참조 검사라는 것이 이 한 줄로 증명된다. 데이터 손실 방향과
정리 방향이 모두 실제 R2와 실제 D1 위에서, 배포된 Worker의 자기 cron으로
확인됐다.

부수 확인: `attachment` 행을 지우려 하자 `bubble`의 `attachment_ref_attachment_id`
외래키가 막았다. 정본 스키마가 M05에서 넣기로 한 제약이 실제로 돌고 있다. 그래서
행을 지우는 대신 `r2_object_key`만 바꿨다.
