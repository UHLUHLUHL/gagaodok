...
object ConversationCompactor {
    ...
    ///
    /// **이 값을 응답 속도 때문에 다시 올리지 마십시오.** 한때 기다리는 시간을 줄이려고
    /// 손댔다가, 실기기에서 입력 13,708토큰에 캐시까지 똑같이 물린 두 요청이 35초 간격으로
    /// 각각 1.7초와 24.3초가 나오는 것을 확인했습니다. 첫 글자까지의 시간은 입력 크기가
    /// 아니라 서버 쪽 변동이 정합니다. 이 값이 정하는 것은 **토큰 비용과 기억의 폭**이지
    /// 속도가 아닙니다.
