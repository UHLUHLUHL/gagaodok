...
object ConversationCompactor {
    ...
    ///
    /// 한때 **기다리는 시간**을 줄이려고 80으로 내린 적이 있습니다. 압축 전에는 대화 전체가
    /// 매 요청마다 나가니 입력이 클수록 느릴 것이라고 봤습니다. **틀렸습니다.** 실기기에서
    /// 재 보니 입력 13,708토큰에 캐시까지 똑같이 물린 두 요청이 35초 간격으로 각각 1.7초와
    /// 24.3초가 나왔습니다. 첫 글자까지의 시간은 입력 크기가 아니라 서버 쪽 변동이 정합니다.
    ///
    /// 근거가 사라졌으므로 되돌립니다. 지연에 도움이 안 되는 값으로 기억을 깎을 이유가 없습니다.
