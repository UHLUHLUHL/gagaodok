...
object ConversationCompactor {
    ...
    ///
    /// 처음에는 **요금**만 보고 150으로 잡았습니다. 손익분기가 60~70턴 언저리라, 그 근처에서
    /// 켜 봐야 아끼는 돈은 없고 기억만 먼저 잃기 때문이었습니다.
    ///
    /// 그런데 요금 말고 **기다리는 시간**이 문제가 됐습니다. 압축이 걸리기 전에는 대화 전체가
    /// 매 요청마다 통째로 나가고, 첫 글자까지 걸리는 시간은 입력 크기에 거의 비례합니다.
    /// 실기기에서 같은 방인데 대화가 짧을 때 3.4초, 몇 시간 쌓인 뒤 23초가 걸렸습니다.
    ///
    /// 그래서 손익분기 위쪽인 80으로 내립니다. 돈은 조금 아끼거나 본전이고, 프롬프트가
    /// 무한정 커지지 않게 막는 것이 목적입니다. 80턴이면 원문 구간이 20~70턴을 오가므로
    /// 최근 대화의 결은 그대로 남습니다.
    ...
    private val MENTOR_SUMMARY_INSTRUCTION = """
...
답변에는 정리한 내용만 담고 인사나 설명을 덧붙이지 않는다.
    ...
    private val COMPANION_SUMMARY_INSTRUCTION = """
...
답변에는 정리한 내용만 담고 인사나 설명을 덧붙이지 않는다.
    ...
    fun transcript(turns: List<ConversationTurn>, startingTurn: Int, mode: ChatMode): String {
        // 부르는 이름도 모드를 따릅니다. 캐릭터 대화를 "학습자/답변자"로 옮겨 놓으면
        // 요약하는 쪽이 그걸 수업 기록으로 읽습니다.
        val userLabel = if (mode == ChatMode.COMPANION) "사용자" else "학습자"
        val botLabel = if (mode == ChatMode.COMPANION) "상대" else "답변자"
        ...
        for (turn in turns) {
            ...
            val who = if (turn.sender == MessageSender.USER) userLabel else botLabel
            var line = turn.text
    ...
    fun render(digest: ConversationDigest, mode: ChatMode): String {
