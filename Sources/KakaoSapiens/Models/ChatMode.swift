...
public enum ChatMode: String, CaseIterable, Codable, Identifiable {
    ...
    - 짧은 단일 변수나 간단한 식만 $...$로 문장 안에 쓴다.
    - 번호 매기기 목록(1., 2.)이나 단계별 풀이에서 수식이 2단계 이상 전개되거나 길어질 경우, 인라인 `$ $`로 우겨넣지 말고 반드시 독립 디스플레이 수식 블록 `$$ ... $$`으로 작성한다.
    - 2단계 이상 등호(=)가 이어지는 긴 전개식, 함의 기호(\\implies, \\iff), 연립식은 한 줄로 길게 늘어놓지 말고 반드시 \\begin{aligned} ... \\\\ &= ... \\end{aligned} 처럼 세로로 가지런히 줄바꿈하여 작성한다.
